...
class RefCOCO(Dataset):
    def __init__(self, image_set, root_path, data_path, boxes='gt', proposal_source='official',
        ...
        coco_annot_files = {
            "train2014": "mscoco/annotations/instances_train2014.json",
            "val2014": "mscoco/annotations/instances_val2014.json",
            "test2015": "mscoco/annotations/image_info_test2015.json",
        }
        proposal_dets = 'refer/refcoco+_unc_gt_imgfeat/res101_coco_minus_refer_notime_dets_36.json'
        ...
        self.vg_proposal = ("mscoco/mscoco_imgfeat/vl-bert", "trainval_obj36-36")
        self.with_precomputed_visual_feat = True  # FIXME
        ...
        self.refer = REFER(os.path.join(data_path, 'refer/data/'), dataset='refcoco+', splitBy='unc')
        self.refer_ids = []
    ...
    def __getitem__(self, index):
        ...
        if self.boxes == 'gt':
            ...
            if self.proposal_source == 'official':
                ...
                zip_mode = False  # FIXME
                box_file = os.path.join(self.data_path, self.vg_proposal[0],
                                        '{0}.zip@/{0}'.format(self.vg_proposal[1])
                                        if zip_mode else self.vg_proposal[1])
    ...
    def group_aspect(database):
        ...
        horz = widths >= heights
        vert = torch.logical_not(horz)  # 1 - horz
        group_ids[horz] = 0
        group_ids[vert] = 1

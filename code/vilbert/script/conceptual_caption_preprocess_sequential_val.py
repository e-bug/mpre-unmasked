...
class Conceptual_Caption(RNGDataFlow):
    ...
    def __init__(self, corpus_path, shuffle=False):
        ...
        #self.num_file = 30
        self.name = os.path.join(corpus_path, 'val.tsv') # FIXME conceptual_caption_trainsubset_resnet101_faster_rcnn_genome.tsv.%d')
        self.infiles = [self.name] #[self.name % i for i in range(self.num_file)]
        self.counts = []

        self.captions = {}
        df = open_tsv(os.path.join(corpus_path, 'Validation_GCC-1.1.0-Validation.tsv'), 'validation')  #'/srv/share/datasets/conceptual_caption/DownloadConceptualCaptions/Train_GCC-training.tsv', 'training')
        ...
        with open(os.path.join(corpus_path, 'caption_val.json'), 'w') as f:
...
if __name__ == '__main__':
    corpus_path = '/home/bugliarello.e/projects/vilbert_beta/data'
    ds = Conceptual_Caption(corpus_path)
    ds1 = PrefetchDataZMQ(ds, nr_proc=1)
    LMDBSerializer.save(ds1, os.path.join(corpus_path, 'validation_feat_all.lmdb'))

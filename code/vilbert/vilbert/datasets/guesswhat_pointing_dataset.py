...
class GuessWhatPointingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mc_indexes = entry["mc_idx"] #+ [36] * 37
        print(str(image_id) + " " + str(mc_indexes))
        multiple_choice_idx = torch.from_numpy(np.array(mc_indexes[:37]))
...
#        gt_features = gt_features[1:gt_num_boxes]
        ...
        mix_boxes_ori = boxes_ori  # np.concatenate((boxes_ori, gt_boxes_ori), axis=0)
        mix_boxes = boxes  # np.concatenate((boxes, gt_boxes), axis=0)
        mix_features = features  # np.concatenate((features, gt_features), axis=0)
        mix_num_boxes = min(int(num_boxes - 1), self.max_region_num)  #min(int(num_boxes + int(gt_num_boxes) - 1), self.max_region_num)

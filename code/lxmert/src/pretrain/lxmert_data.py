...
from utils import load_obj_tsv, load_lines_obj_tsv

TINY_IMG_NUM = 500
FAST_IMG_NUM = 5000

BASEDIR = '/data/bugliarello.e/data/conceptual_captions/'
...
Split2ImgFeatPath = {
    ...
    'concap_train': BASEDIR + 'train_obj36-36.clean.tsv',
    'concap_valid': BASEDIR + 'valid_obj36-36.clean.tsv',
}
NUM_MINISETS = 181
for i in range(NUM_MINISETS):
    n = str(i)
    Split2ImgFeatPath['concap_train_'+n] = BASEDIR + 'train_obj36-36.clean.%s.tsv' % n.zfill(3)
...
class LXMERTDataset:
    def __init__(self, splits: str, qa_sets=None):
        ...
        for source in self.sources:
            self.data.extend(json.load(open(BASEDIR + "lxmert/caption_%s.json" % source.split('_')[1])))
...
class IXTorchDataset(Dataset):
    def __init__(self, splits: str):
        super().__init__()
        sources = splits.split(',')
        captions = []
        for source in sources:
            captions.extend(json.load(open(BASEDIR + "lxmert/caption_%s.json" % source.split('_')[1])))

        self.ixs = np.arange(0, len(captions))
        print("Load %d captions from %s" % (len(self.ixs), source))

    def __len__(self):
        return len(self.ixs)

    def __getitem__(self, item: int):
        return self.ixs[item]


class LXMERTIXTorchDataset(Dataset):
    def __init__(self, dataset: LXMERTDataset, lines: int, topk=-1):
        super().__init__()
        self.raw_dataset = dataset
        self.task_matched = args.task_matched
        ...
        img_data = []
        for source in self.raw_dataset.sources:
            img_data.extend(load_lines_obj_tsv(Split2ImgFeatPath[source], lines, topk))

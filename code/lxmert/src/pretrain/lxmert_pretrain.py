...
from pretrain.lxmert_data import InputExample, LXMERTDataset, LXMERTTorchDataset, LXMERTEvaluator, NUM_MINISETS
...
# def get_subset_tuple(splits: str, bs: int, lines: list, shuffle=False, drop_last=False, topk=-1) -> DataTuple:
#     # Decide which QA datasets would be used in pre-training.
#     # Options: vqa, gqa, visual7w
#     # Note: visual7w is a part of vgqa, we take the name here.
#     qa_sets = args.qa_sets
...
#     # Build dataset, data loader, and evaluator.
#     dset = LXMERTDataset(splits, qa_sets=qa_sets)
#     tset = LXMERTIXTorchDataset(dset, lines, topk)
#     data_loader = DataLoader(
#         tset, batch_size=bs,
#         shuffle=shuffle, num_workers=args.num_workers,
#         collate_fn=lambda x: x,
...
def random_feat(feats, torchdset):
    ...
    for i in range(len(feats)):
        ...
        if prob < args.obj_mask_rate:
            ...
            # 10% randomly change token to random feat
                ...
                mask_feats[i, :] = torchdset.random_feat()
...
def convert_example_to_features(example: InputExample, max_seq_length, tokenizer, torchdset)->InputFeatures:
    """
    ...
    # Mask Image Features:
    masked_feat, feat_mask = random_feat(feat, torchdset)
...
class LXMERT:
    ...
    def forward(self, examples, torchdset):
        train_features = [convert_example_to_features(example, self.max_seq_length, self.tokenizer, torchdset)
    ...
    def train_batch(self, optim, batch, torchdset):
        optim.zero_grad()
        loss, losses, ans_logit = self.forward(batch, torchdset)
        if args.multiGPU:
            loss = loss.mean()
    ...
    def valid_batch(self, batch, torchdset):
        with torch.no_grad():
            loss, losses, ans_logit = self.forward(batch, torchdset)
            if args.multiGPU:
                loss = loss.mean()
    ...
    def train(self):  #, eval_tuple: DataTuple):
        ...
        batch_per_epoch = len(train_dset) // args.batch_size + bool(len(train_dset) % args.batch_size)
        ...
        for epoch in range(args.epochs):
            ...
            for i in tqdm(range(NUM_MINISETS), total=NUM_MINISETS):
                ...
                dtuple = get_tuple(args.train + '_%d' % i, args.batch_size, shuffle=True, drop_last=True)
                dl = dtuple.loader
                for batch in tqdm(dl, total=len(dl)):
                    loss, losses, logit = self.train_batch(optim, batch, dtuple.torchdset)
                    total_loss += loss
                    total_losses += losses
            ...
            # Eval
            dtuple = get_tuple(args.valid, valid_batch_size, shuffle=False, drop_last=False) #, topk=5000)
    ...
    def evaluate_epoch(self, eval_tuple: DataTuple, iters: int=-1):
        self.model.eval()
        eval_dl = eval_tuple.loader
        total_loss = 0.
        total_losses = 0.
        uid2ans = {}
        for i, batch in enumerate(eval_dl):
            loss, losses, logit = self.valid_batch(batch, eval_tuple.torchdset)
            total_loss += loss
            total_losses += losses
            if args.task_qa:
                score, label = logit.max(1)
                for datum, l in zip(batch, label.cpu().numpy()):
                    uid = datum.uid
                    ans = train_dset.answer_table.id2ans(l)
                    uid2ans[uid] = ans
            if i == iters:
                break

        print("The valid loss is %0.4f" % (total_loss / len(eval_dl)))
        losses_str = "The losses are "
        for name, loss in zip(LOSSES_NAME, total_losses / len(eval_dl)):

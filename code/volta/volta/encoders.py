...
class BertForVLTasks(BertPreTrainedModel):
    def __init__(self, config, task_cfg, task_ids, dropout_prob=0.1):
        ...
        for task_id in task_ids:
            ...
            task_clf = "TASK" + str(task_cfg[task_id]["task_id"]).strip()
            if task_type in {"VL-classifier", "VL-classifier-GQA"}:
                task2clf[task_clf] = SimpleClassifier(config.pooler_size, config.clf_hidden_size,
                                                      task_cfg[task_id]["num_labels"])
            elif task_type == "VL-binary-classifier":
                task2clf[task_clf] = SimpleClassifier(config.pooler_size * 2, config.clf_hidden_size, 2)
            elif task_type == "VL-tri-classifier":
                task2clf[task_clf] = nn.Linear(config.pooler_size, 3)  # for Visual Entailiment tasks
            elif task_type == "VL-logit":
                task2clf[task_clf] = nn.Linear(config.pooler_size, 1)
            elif task_type.startswith("V-logit"):
                if task_cfg[task_id].get("num_clf_layers", 1) == 2:
                    task2clf[task_clf] = torch.nn.Sequential(
                    ...
                    task2clf[task_clf] = nn.Linear(config.v_hidden_size, 1)
            else:
    ...
    def forward(
        ...
        task_clf = "TASK" + str(self.task_cfg[task_id]["task_id"]).strip()
        if self.task_cfg[task_id]["type"].startswith("V-logit"):
            vil_prediction = self.clfs_dict[task_clf](self.dropout(sequence_output_v)) + (
            ...
            # NLVR
            vil_prediction = self.clfs_dict[task_clf](pooled_output.view(-1, pooled_output.size(1) * 2))
        else:
            vil_prediction = self.clfs_dict[task_clf](pooled_output)
